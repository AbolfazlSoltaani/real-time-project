...
# speedup_factor_list = [1.5]
for speedup_factor in speedup_factor_list:
    cedf = CriticallyEDF(
        tasks=tasks, resources=resources, speedup_factor=speedup_factor, verbose=True, overrun_chance=30
    )
    if cedf.schedule():
        cedf.visualize(
            show=False, save=True, filename=f"overrun-speedup{speedup_factor}"
        )
